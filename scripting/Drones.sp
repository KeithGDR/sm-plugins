...
#define MODEL_DRONE	  	"models/props_survival/drone/br_drone.mdl"
#define MODEL_THRUSTER 	"models/props/cs_office/trash_can_p8.mdl"
#define MODEL_MISSILE 	"models/shells/shell_338mag.mdl"

char g_DroneGibModels[4][64] = {
...
#define SOUND_DRONE	 	  "#/vehicles/drone_loop_03.wav"
#define SOUND_DRONE_BREAK "weapons/taser/taser_shoot.wav"

#define SOUND_ROCKET_FIRED 		  "survival/missile_land_01.wav"
#define SOUND_ROCKET_FIRED_HOMING "survival/missile_land_04.wav"
#define SOUND_ROCKET_FLY 		  "ambient/nuke/vent_02.wav"
#define SOUND_ROCKET_EXPLODE 	  "weapons/c4/c4_explode1.wav"
...
ConVar g_cvSelfDestruct;
ConVar g_cvTurnForce;

ConVar g_cvReloadInterval;
ConVar g_cvMissileDamage;
ConVar g_cvMissileRadius;
...
int g_Drone[MAX_DRONES] = 	   {-1, ...};
int g_DroneView[MAX_DRONES] =  {-1, ...};
int g_DroneUI[MAX_DRONES] =    {-1, ...};
int g_DroneSound[MAX_DRONES] = {-1, ...};
int g_DroneThrusts[MAX_DRONES][TOTAL_THRUSTERS];
int g_DroneSelectedView[MAX_DRONES];

/************************************/

int g_pInDrone[MAXPLAYERS + 1] = {-1, ...};

float g_pActivateLocation[MAXPLAYERS + 1][2][3];

float g_pFireMissileTimer[MAXPLAYERS + 1];
float g_pGeneralTimer[MAXPLAYERS + 1];
...
public void OnMapStart()
{
	...
	for (int i = 0; i < 4; i++) {
		PrecacheModel(g_DroneGibModels[i], true);
	}
	
	g_HaloSprite = PrecacheModel(SPRITE_HALO, true);
	g_BeamSprite = PrecacheModel(SPRITE_BEAM, true);
	
	PrecacheSound(SOUND_DRONE, true); 
	PrecacheSound(SOUND_DRONE_BREAK, true);
...
{
	if (g_pInDrone[client] > -1)
		ExitDrone(client, true);
}
...
{	
	...
	for (int i = 0; i < target_count; i++) {
		CreateDrone(client, target_list[i]);
	}
...
stock Action CreateDrone(int client, int target)
{
	...
	if (DispatchSpawn(drone)) {
		g_Drone[dIndex] = drone;
		SDKHook(drone, SDKHook_TouchPost, DroneTouchPost);
	}
	...
	DispatchKeyValue(sound, "message", SOUND_DRONE);
...
public Action DroneTouchPost(int entity, int other) {
	if (entity != other) {
		int pilot = GetPilot(entity);
		if (pilot != -1) {
...
int GetPilot(int entity) {
	for (int i = 1; i <= MaxClients; i++) {
		if (IsClientInGame(i) && g_pInDrone[i] > -1) {
			if (g_pInDrone[i] == entity) {
				return g_Drone[entity];
			}
		}
	}
...
stock void CreateThrustEntities(int dIndex, int drone)
{
	for (int i = 0; i < TOTAL_THRUSTERS; i++) {
		...
		if (i > 0 && i < TOTAL_THRUSTERS - 2) {
			...
			char nameThrustProp[32];
			Format(nameThrustProp, sizeof(nameThrustProp), "%d_thrustprop_%d", dIndex, i);
			DispatchKeyValue(thrustprop, "targetname", nameThrustProp);
			
			char spawnflagsThrustProp[32];
			Format(spawnflagsThrustProp, sizeof(spawnflagsThrustProp), "%i", 2 | 4 | 128 | 512 | 1024); // No Damage | No Collide | No Rotorwash | No Pickup | No Motion on Bump
		...
		char nameThrust[32];
		Format(nameThrust, sizeof(nameThrust), "%d_thruster_%d", dIndex, i);
		DispatchKeyValue(thrust, "targetname", nameThrust);
		
		char spawnflagsThrust[32];
		if (i == 6 || i == 7) Format(spawnflagsThrust, sizeof(spawnflagsThrust), "%i", 4 | 8 | 16); // Torque(Rotation) | Local | Ignore Mass
		...
		if (i == 0) { // 0: UPWARD
			...
			char turnforce[18];
			g_cvTurnForce.GetString(turnforce, sizeof(turnforce));
			DispatchKeyValue(thrust, "force", turnforce);
		}
...
{
	if (IsValidEntity(client) && IsPlayerAlive(client) && dIndex > -1 && g_Drone[dIndex] > 0 && IsValidEntity(g_Drone[dIndex])) {	
		// Reset timers
...
stock Action ExitDrone(int client, bool explode)
{
	...
	if (IsValidEntity(client) && dIndex > -1 && IsValidEntity(g_Drone[dIndex])) {
		...
		for (int i = 0; i < TOTAL_THRUSTERS; i++) {
			AcceptEntityInput(g_DroneThrusts[dIndex][i], "Deactivate");
		}
		...
		if(explode && g_cvSelfDestruct.BoolValue == true) {
			float pos[3], ang[3];
...
stock void CreateDroneExplosion(int owner, float pos[3], float ang[3])
{
	for (int i = 0; i < 4; i++) {
		...
		DispatchKeyValue(gib, "model", g_DroneGibModels[i]);
	...
	CreateExplosion(owner, pos, "15.0", SOUND_DRONE_BREAK);	
}
...
public void OnGameFrame() 
{
	for (int i = 1; i <= MaxClients; i++) {
		// Not in drone
		if (IsValidEntity(i) && IsClientInGame(i) && IsPlayerAlive(i) && g_pInDrone[i] < 0)
			SetEntityRenderMode(i, RENDER_NORMAL); //sad attempt to restore enemy visibility glitch
			
		// In drone
		if (IsValidEntity(i) && IsClientInGame(i) && IsPlayerAlive(i) && g_pInDrone[i] > -1) {
			int pilot = i;
			int dIndex = g_pInDrone[pilot];
			...
			if (g_Drone[dIndex] > 0 && IsValidEntity(g_Drone[dIndex])) {
				...
				if (keypress & IN_SPEED && (GetGameTime() - g_pGeneralTimer[pilot] >= 0.75)) {
					...
					// This stays up here
					if (g_DroneSelectedView[dIndex] == 1)
						viewang[0] += 12.5;
					...
					if (g_DroneSelectedView[dIndex] == 0)
						viewang[0] -= 12.5;
				...
				if (((keypress & IN_ATTACK) && (keypress & IN_ATTACK2) || (keypress & IN_RELOAD)) && (GetGameTime() - g_pFireMissileTimer[pilot] >= g_cvReloadInterval.FloatValue)) {
					...
					if (target > 0 && target <= MaxClients && IsValidEntity(target) && IsPlayerAlive(target)) {
						...
						EmitSoundToAll(SOUND_ROCKET_FIRED_HOMING, g_Drone[dIndex]);		
					}
					else {
						...
						EmitSoundToAll(SOUND_ROCKET_FIRED, g_Drone[dIndex]);
					}
...
stock int CreateMissileProp(const char[] name)
{
	...
	if (DispatchSpawn(missile))
		SetEntityRenderColor(missile, 220, 120, 0, 255);
...
public void OnPassCollisionTrack(const char[] output, int track, int train, float delay)
{
	...
	if (IsValidEntity(train) && IsValidEntity(track)) {	
		...
		// Explode on pass
		CreateExplosion(pilot, explodepos, "500.0", SOUND_ROCKET_EXPLODE);
...
public void OnMissileFrame(const char[] output, int train, int track, float delay)
{
	if (IsValidEntity(train) && IsValidEntity(track)) {
		...
		if (IsValidEntity(pilot) && IsValidEntity(missile)) {
			...
			float mins[3] = { -2.0, -2.0, -1.5 };
			float maxs[3] = { 2.0, 2.0, 1.5 };
			...
			if(TR_DidHit(INVALID_HANDLE)) {
				...
				CreateExplosion(pilot, hitpos, "500.0", SOUND_ROCKET_EXPLODE);
...
// ****************** EXPLOSION CREATION *****************
//--------------------------------------------------------
stock int CreateExplosion(int owner, float pos[3], char[] damageforce, char[] sound)
{
...
stock Action TimerDeleteEntity(Handle timer, int entref)
{
	if (IsValidEntRef(entref))
		AcceptEntityInput(EntRefToEntIndex(entref), "Kill");
