...
public Plugin:myinfo = {
    name = "Force Join",
    author = "floube - Fixed by Zephyrus",
...
public OnPluginStart() {
    ...
    cvarForceTeamOnDeath = CreateConVar("sm_force_team_on_death", "BLU");
...
public OnMapStart() {
    ...
    for (new i = 1; i <= MaxClients; i++) {
        Format(forcedTeams[i], sizeof(forcedTeams[]), "UNDEFINED");
    }
}

public resetClient(client) {
    Format(forcedTeams[client], sizeof(forcedTeams[]), "UNDEFINED");
}
...
public OnClientPostAdminCheck(client) {
    if (client != 0 && client != -1) {
        ...
        if (StrEqual(strForceTeamOnJoin, "BLU")) {
            ...
            Format(forcedTeams[client], sizeof(forcedTeams[]), "BLU");
        } else if (StrEqual(strForceTeamOnJoin, "RED")) {
            ChangeClientTeam(client, TEAM_RED);
            Format(forcedTeams[client], sizeof(forcedTeams[]), "RED");
        } else if (StrEqual(strForceTeamOnJoin, "SPEC")) {
            ChangeClientTeam(client, SPECTATOR);
            Format(forcedTeams[client], sizeof(forcedTeams[]), "SPEC");
        }
    }
}
...
public Action:eventRoundWin(Handle:event, const String:name2[], bool:dontBroadcast) {
    for (new i = 1; i <= MaxClients; i++) {
        Format(forcedTeams[i], sizeof(forcedTeams[]), "RED");
    }
}
