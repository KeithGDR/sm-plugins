...
public void OnClientSayCommand_Post(int author, const char[] command, const char[] sArgs)
{
...
{
	char clientIndex[4];
	IntToString(client, clientIndex, sizeof(clientIndex));
	Handle menu = CreateMenu(MenuHandle_YesNo);
	SetMenuTitle(menu, "%N want to fight you in a 1v1 duel !", client);
	AddMenuItem(menu, clientIndex, "Yes - I'm a true Warrior");
	AddMenuItem(menu, clientIndex, "No - I'm a coward");
	SetMenuExitButton(menu, true);
	DisplayMenu(menu, enemy, MENU_TIME_FOREVER);
}
